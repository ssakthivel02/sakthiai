...
let result=await evaluateSignedAgentWebhook(await signedRequest(),{...env,AGENT_WEBHOOK_ENABLED:'false'},NOW);
assert.equal(result.code,'AGENT_WEBHOOK_DISABLED');
assert.equal(result.status,503);

result=await evaluateSignedAgentWebhook(await signedRequest(),{...env,AGENT_WEBHOOK_SHARED_SECRET:'short'},NOW);
assert.equal(result.code,'AGENT_WEBHOOK_SECRET_MISSING_OR_WEAK');

result=await evaluateSignedAgentWebhook(await signedRequest(),{...env,AGENT_WEBHOOK_ALLOWED_AGENTS:''},NOW);
...
result=await evaluateSignedAgentWebhook(await signedRequest({agentId:'not-allowed'}),env,NOW);
assert.equal(result.code,'AGENT_WEBHOOK_AGENT_NOT_ALLOWED');
assert.equal(result.status,403);

result=await evaluateSignedAgentWebhook(await signedRequest({tenantId:'tenant-not-allowed'}),env,NOW);
assert.equal(result.code,'AGENT_WEBHOOK_TENANT_NOT_ALLOWED');
assert.equal(result.status,403);

const stale=String(Math.floor(NOW/1000)-301);
result=await evaluateSignedAgentWebhook(await signedRequest({timestamp:stale}),env,NOW);
assert.equal(result.code,'AGENT_WEBHOOK_TIMESTAMP_STALE');
assert.equal(result.status,401);

result=await evaluateSignedAgentWebhook(await signedRequest({signatureOverride:'v1='+'0'.repeat(64)}),env,NOW);
...
result=await evaluateSignedAgentWebhook(await signedRequest({body:tamperedBody,runId:'run_tamper',nonce:'nonce_tamper',signatureOverride:originalSignature}),env,NOW);
assert.equal(result.code,'AGENT_WEBHOOK_SIGNATURE_INVALID');

const valid=await evaluateSignedAgentWebhook(await signedRequest(),env,NOW);
...
assert.equal(serializedValid.includes(valid.transport.signature||'__never__'),false);
...
const replay=await evaluateSignedAgentWebhook(await signedRequest(),env,NOW);
...
result=await evaluateSignedAgentWebhook(await signedRequest({body:writeBody,runId:'run_write',nonce:'nonce_write'}),env,NOW);
assert.equal(result.code,'AGENT_WEBHOOK_P1_READ_ONLY_REQUIRED');
assert.equal(result.status,403);

const executeBody=bodyOf({...proposal,requestedExecution:'execute'});
result=await evaluateSignedAgentWebhook(await signedRequest({body:executeBody,runId:'run_execute',nonce:'nonce_execute'}),env,NOW);
...
url=new URL(URL);
response=await handleAgentApi(await signedRequest({runId:'run_route',nonce:'nonce_route'}),env,url,'req_eval');
assert.equal(response.status,200);
