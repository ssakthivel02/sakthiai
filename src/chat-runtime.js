...
function boundedTokens(base,admission){
  const requested=Math.max(1,Number.parseInt(String(base??700),10)||700);
  const scale=Number(admission?.degradation?.maxOutputScale??1);
  if(!Number.isFinite(scale)||scale<=0||scale>=1)return requested;
  return Math.max(1,Math.floor(requested*scale));
}
...
export async function runChatModel(env={},options={}){
  ...
  const execution=await executeWithAdmission(env,{
    requestClass:'interactive',
    priority:'normal',
    cacheable:Boolean(options.cacheable)
  },async admission=>{
    const maxTokens=boundedTokens(baseMaxTokens,admission);
  ...
  return {
    ...
    maxTokens:execution.value.maxTokens,
    admission:execution.admission,
    capacityScope:execution.signal?.scope||'worker-isolate',
