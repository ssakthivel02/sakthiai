...
function queued(input={}){
  ...
  return requestClass==='batch'||priority==='low';
}
...
export async function executeWithAdmission(env={},input={},run){
  ...
  if(!reservation.ok){
    const shouldQueue=queued(input);
    return {
      ok:false,
      executed:false,
      signal:reservation,
      admission:{
        ...admission,
        ok:false,
        decision:shouldQueue?'QUEUE':'REJECT',
        code:reservation.code||'CAPACITY_RESERVATION_FAILED',
        retryAfterSeconds:shouldQueue?20:15
      }
    };
  }

  try{
    const value=await run(admission);
    return {ok:true,executed:true,value,admission,signal:reservation};
  }finally{
    reservation.release();
  }
}
